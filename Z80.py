      ...
      do {
        ...
        if ( interruptTriggered( _local_tstates ) ) {
          break;
			...
			#	mem[dest+index] = mem[src+index]
			mem[dest & 0xffff: (dest + count) & 0xffff] = mem[src & 0xffff : (src + count) & 0xffff]
			count = 0 & 0xffff
			dest = (dest + count) & 0xffff
			src = (src + count) & 0xffff
			_local_tstates += 21 #(21*count)
			_R += 2 #(2*count)"""
